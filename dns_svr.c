...
void handle_req_res(unsigned char *full_msg, header_t *header,question_t *question);
...
int main(int argc, char** argv) {
	...
	while(1){
		...
		* Get back a new file descriptor to communicate on */
		...
		// //-------------- IF VALID DO EVERYTHING BELOW, IF NOT HANDLE WITH RCODE & PUT EVERYTHING TO RES AND REQ STRUCT
		question_t *question;
		question = (question_t*)malloc(sizeof(*question));
		assert(question!=NULL);
		handle_req_res(query_msg,header,question);

		printf("heade flags bef %02x %02x\n",header->flags[0],header->flags[1]);

		if(is_non_AAAA(question)){
			rcode_four_error(query_msg);
			printf("+++++++heade flags after rcode 4 %02x %02x\n",query_msg[2],query_msg[3]);
			...
			handle_req_res(upstream_res_msg,header,question);
...
void handle_req_res(unsigned char *full_msg, header_t *header,question_t *question){
	...
	// if(is_non_AAAA(question)){
	// 	rcode_four_error(header->flags);
	// };

	printf("header flags besome %02x %02x\n",header->flags[0],header->flags[1]);
